...
sub _doEvent {
	...
	foreach my $event (@{$_}) {
		...
		#
		$g1 ||= $self->{_impl}->latest_g1_location;
		if ($self->{_impl}->findInProgress($g1, undef, DOT_COMPLETION, undef, 'directDeclarator', ['IDENTIFIER']) &&
		    $self->{_impl}->findInProgress($g1, undef,              2, undef, 'declaration',      ['declarationSpecifiers', 'initDeclaratorList', 'SEMICOLON'])) {
		    #
		    ...
		    if ($self->{_impl}->findInProgress($g1, undef, 1, undef, 'structDeclarator', [ 'declarator', 'COLON', 'constantExpression' ]) ||
			$self->{_impl}->findInProgress($g1, undef, 1, undef, 'structDeclarator', [ 'declarator' ])) {
...
sub _doLexeme {
	...
	$g1 ||= $self->{_impl}->latest_g1_location;
	if ($self->{_impl}->findInProgress($g1, undef, DOT_PREDICTION, undef, 'typeSpecifier', [ 'TYPEDEF_NAME' ]) && $self->{_scope}->parseIsTypedef($lexeme_value) && $self->_canEnterTypedefName($g1)) {
	    $lexeme = 'TYPEDEF_NAME';
	} elsif ($self->{_impl}->findInProgress($g1, undef, DOT_PREDICTION, undef, 'constant', [ 'ENUMERATION_CONSTANT' ]) && $self->{_scope}->parseIsEnum($lexeme_value) && $self->_canEnterEnumerationConstant($g1)) {
...
sub _canEnterTypedefName {
    ...
    my $rc = 1;
    if ($self->{_impl}->findInProgress($g1, undef, 1, undef, 'parameterDeclaration', ['declarationSpecifiers', 'declarator'])) {
	#
...
sub _canReenterScope {
    ...
    return ($self->{_impl}->findInProgress($g1 - 1, undef, 3, undef, 'functionDefinition', [ 'declarationSpecifiers', 'declarator', 'declarationList', 'compoundStatement' ]) ||
	    $self->{_impl}->findInProgress($g1 - 1, undef, 2, undef, 'functionDefinition', [ 'declarationSpecifiers', 'declarator', 'compoundStatement' ]));
}
